...
public class MapActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        boatMap.addMarker(new MarkerOptions().position(lebato).title("Bateau : " + boat.getCaptain_name() + " " +boat.getBoat_name()).snippet(boat.getLatitude() + ", " + boat.getLongitude()).icon(BitmapDescriptorFactory.fromResource(R.drawable.boat2)));
        boatMap.addMarker(new MarkerOptions().position(lepor).title("Départ : " + boat.getDepart().getNom_port()).snippet(boat.getDepart().getLatitude() + ", " + boat.getDepart().getLongitude()).icon(BitmapDescriptorFactory.fromResource(R.drawable.anchor)));
